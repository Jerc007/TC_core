...
entity stage_decode_add is
    port (
			...
			X_sgn : 	out std_logic;
			X_not_zero: out std_logic;
			X_nar: 		out std_logic;
			Y_sf : 		out std_logic_vector(7 downto 0);
			Y_f : 		out std_logic_vector(26 downto 0);
			Y_sgn : 	out std_logic;
			Y_not_zero: out std_logic;
			Y_nar : 	out std_logic );
end entity;

architecture arch of stage_decode_add is
   ...
   component PositFastDecoder_32_2_F0_uid8 is
	...
	signal X_not_zero_s: 	std_logic;
	signal X_nar_s: 		std_logic;
	signal Y_sgn_s: 		std_logic;
	signal Y_sf_s: 		std_logic_vector(7 downto 0);
	signal Y_f_s: 			std_logic_vector(26 downto 0);
	signal Y_not_zero_s: 	std_logic;
	signal Y_nar_s: 		std_logic;
...
begin
	...
	if rising_edge(clk_in) then
		if reset = '1' then
			...
			X_sf  		<= X_sf_s;
			X_f   		<= X_f_s;
			X_not_zero 	<= X_not_zero_s;
			X_nar		<= X_nar_s;
			Y_sgn		<= Y_sgn_s;
			Y_sf  		<= Y_sf_s;
			Y_f			<= Y_f_s;
			Y_not_zero 	<= Y_not_zero_s;
			Y_nar		<= Y_nar_s;
		end if;
	  end if;
	end process;
	...
	--------------------------- Check for Zeros and NaRs ---------------------------
	   X_not_zero_s <= X_sgn_s OR X_nzn_s;
	   X_nar_s <= X_sgn_s AND NOT(X_nzn_s);
	   Y_not_zero_s <= Y_sgn_s OR Y_nzn_s;
	   Y_nar_s <= Y_sgn_s AND NOT(Y_nzn_s);
...
entity stage_compare is
    port (
			...
			X_sgn : 	in std_logic;
			X_not_zero: in std_logic;
			Y_sf : 		in std_logic_vector(7 downto 0);
			Y_sgn : 	in std_logic;
			Y_not_zero: in std_logic;
			add_frac : 	out std_logic_vector(29 downto 0);
			grd_tmp : 	out std_logic;
			rnd_tmp : 	out std_logic;
			stk_tmp : 	out std_logic;
			larger_sf_out: out std_logic_vector(7 downto 0)
            );			
end entity;

architecture arch of stage_compare is
   ...
   component RightShifterSticky31_by_max_31_F0_uid12 is
	...
	signal grd_tmp_s :  std_logic;
	signal rnd_tmp_s :  std_logic;
	signal stk_tmp_s :  std_logic;
	signal add_frac_s :  std_logic_vector(29 downto 0);
...
---------------- Compute exponents difference & align fractions ----------------
   ...
   smaller_frac_sh <= shifted_frac(30 downto 2);
	...
	grd_tmp_s <= shifted_frac(1);
	rnd_tmp_s <= shifted_frac(0);
	add_frac_s <= (larger_frac(28) & larger_frac) + (smaller_frac_sh(28) & smaller_frac_sh); 
	larger_sf_out <= larger_sf;
...
entity stage_fraction_rounding is
    port (
			clk_in	: 	in std_logic;
			reset	:	in std_logic;
			add_frac: 	in std_logic_vector(29 downto 0);
			grd_tmp : 	in std_logic;
			rnd_tmp : 	in std_logic;
			stk_tmp : 	in std_logic;
			X_nar	: 	in std_logic;
			Y_nar 	: 	in std_logic;
			larger_sf:	in   std_logic_vector(7 downto 0);
			R 		: 	out std_logic_vector(31 downto 0)
			);
end entity;

architecture arch of stage_fraction_rounding is
...
component stage_fraction_rounding is
    port (
			clk_in	: 	in std_logic;
			reset	:	in std_logic;
			add_frac: 	in std_logic_vector(29 downto 0);
...
signal larger_sf_out_s:  std_logic_vector(7 downto 0);
...
valid_out <= valid_pipe(PIPE_DEPTH-1);
	...
	DECODE: stage_decode_add port map (
			clk_in 	=> clk_in,
			reset 	=> reset,
			X  		=> X,
			Y  		=> Y,
	...
	COMPARE: stage_compare port map (
			clk_in 		=> clk_in,	
			reset 		=> reset,
			X_f  	=> X_f,
			Y_f  	=> Y_f,
			...
			rnd_tmp  	=> rnd_tmp,
			stk_tmp  	=> stk_tmp,
			larger_sf_out	=> larger_sf_out_s
			);

	FRACTION_ROUNDING: stage_fraction_rounding port map(
			clk_in		=> clk_in,
			reset		=> reset,
			add_frac	=> add_frac,
			grd_tmp 	=> grd_tmp,
